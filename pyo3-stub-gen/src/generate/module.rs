...
impl<'a> fmt::Display for ModuleFormatter<'a> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if !module.doc.is_empty() {
            docstring::write_docstring(f, &module.doc, "")?;
        }
        writeln!(f)?;

        let mut imports = module.import();

        // Conditionally add TypeAlias import
        if !use_type_statement && !module.type_aliases.is_empty() {
        ...
        let any_overloaded = module.function.values().any(|functions| {
        ...
        for import_ref in imports.into_iter().sorted() {
            match import_ref {
                ImportRef::Module(module_ref) => {
                    let name = module_ref.get().unwrap_or(&module.default_module_name);
                    if name != module.name && !name.is_empty() {
                        ...
                        let is_internal_module =
                            if let Some(root) = module.default_module_name.split('.').next() {
                        ...
                        if is_internal_module && name.contains('.') {
                            ...
                            if !module.submodules.contains(child_module) {
                ...
                ImportRef::Type(type_ref) => {
                    let module_name = type_ref.module.get().unwrap_or(&module.default_module_name);
        ...
        let mut sorted_re_exports = module.module_re_exports.clone();
        sorted_re_exports.sort_by(|a, b| a.source_module.cmp(&b.source_module));
        for re_export in &sorted_re_exports {
            if re_export.items.is_empty() {
                continue;
        ...
        module.write_all_list(f)?;

        writeln!(f)?;

        // Generate type aliases with configuration
        for alias in module.type_aliases.values() {
            alias.fmt_with_config(&module.name, f, *use_type_statement)?;
            writeln!(f)?;
        }

        // Generate variables
        for var in module.variables.values() {
            var.fmt_for_module(&module.name, f)?;
            writeln!(f)?;
        }

        // Generate classes
        for class in module.class.values().sorted_by_key(|class| class.name) {
            class.fmt_for_module(&module.name, f)?;
        }

        // Generate enums
        for enum_ in module.enum_.values().sorted_by_key(|enum_| enum_.name) {
            enum_.fmt_for_module(&module.name, f)?;
        }

        // Generate functions
        for functions in module.function.values() {
            ...
            for function in sorted_functions {
                ...
                function.fmt_for_module(&module.name, f)?;
            }
        }
